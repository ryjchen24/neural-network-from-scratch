...
fig, subplots = plt.subplots(4, 5, figsize=(12, 8))
...
for i, axes in enumerate(subplots.flat):
    ...
    pred_label = class_names[nn.predict(X_test[indices[i]:indices[i]+1])[0]]
    true_label = class_names[np.argmax(y_test[indices[i]])]
    title_text = f"Pred: {pred_label} | True: {true_label}"

    axes.imshow(img, cmap="gray")
    if pred_label == true_label:
      axes.set_title(title_text, fontsize=8, c="g", pad=5) # green text if correct
    else:
      axes.set_title(title_text, fontsize=8, c="r", pad=5) # red text if wrong
    axes.axis(False)
plt.show()